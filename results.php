...
<?php include 'header.php';?>
 <?php
    ...
    echo "<input type='hidden' name='username'></input>";
    echo "<input type='hidden' name='hasLoggedIn'></input>";
    echo "</form>"
 ?>
  ...
  &#9834;
    ...
    <script>
      ...
      document.getElementById("concerts").style.display="none"; 
        ...
        //gets all of the user's top artists (short-term, medium term, and long-term) 
        //and adds them to myArtists and myArtistsIDs
            ...
            headers: {
              ...
              for(let q=0; q<myData.length; q++){
                ...
                if(!myArtistsIDs.includes(curID)){
                  ...
                  if(terms[i]=='long_term'){myArtistsTerms.push("l")}
                  if(terms[i]=='medium_term'){myArtistsTerms.push("m")}
                  if(terms[i]=='short_term'){myArtistsTerms.push("s")}
            ...
            headers: {
              ...
              for(let h=0; h< myData.length; h++){
                ...
                for(let q=0; q < song_artists.length; q++){
                  ...
                  curName= curName.replace(/,/g, "");//getting rid of commas in names so session storage doesnt accidentally make an extra term

                  if(!myArtistsIDs.includes(curID)){
                    
                    curPics=song_artists[q].images;
        ...
        async function getRelatedArtists(id, numRelated){
          ...
          function getArtistArray(){
            arr=[myArtists,myArtistsIDs,myArtistsPics, myArtistsTerms];
            return arr;
          }
          ...
          async function getProfilePic(){
            ...
            headers: {
              ...
              const data = await response.json();
              spotify_name= data.display_name;//getting the spotify username from the data

              //trying to get the users profile pic
               try{myPic=data.images[1].url;}
               catch{myPic='images/blankArtists.png';}
              console.log(myPic)

              //setting the users spotify pic and name in session storage so
              sessionStorage.setItem("userPic",myPic);
              sessionStorage.setItem("displayName", spotify_name);
            }
        ...
        async function getMyArtists() {
          ...
          async function sendMyData(){
            if(sessionStorage.getItem('userArtists')==null){
              ...
              sessionStorage.setItem('userArtistsPics', dataArr[2]);
              sessionStorage.setItem('userArtistsTerms', dataArr[3]);
              ...
              document.forms["artistDataForm"]["artistPics"].value = dataArr[2];
              document.forms["artistDataForm"]["artistTerms"].value = dataArr[3];
            
              document.forms["artistDataForm"]["hasLoggedIn"].value = 1;

              usr = sessionStorage.getItem('username');

              document.forms["artistDataForm"]["username"].value = usr;
